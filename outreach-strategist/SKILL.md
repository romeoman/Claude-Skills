...
| 2b  | Relationship layer usable. Read `stale` and `degraded` as SEPARATE verdicts: `stale: true` means the cache is too old to trust → FAIL. `degraded: true` alone means a source errored upstream while the cache stayed current → **report it with the per-source `error` text, do NOT fail the row**. As of 2026-09-01 both live errors are on graph.one's side (a Postgres deadlock in their email ingestion; LinkedIn 404 "no data for this domain") with 116k emails + 16k LinkedIn items cached and neither needing re-auth | `GET /api/outreach/relationships/status` on Mission Control |
